...
copy %MY_DIR%..\src\main\lib\wpiutil-cpp-2023.2.1-linuxarm64.zip %MY_DIR%contents\wpiutil-cpp-2023.2.1-linuxarm64.zip
:: unzip wpiutil package
powershell expand-archive %MY_DIR%contents\wpiutil-cpp-2023.2.1-linuxarm64.zip -DestinationPath %MY_DIR%contents\
:: build docker image
docker build --platform linux/arm64 -t nt2ros .
:: save docker image to a .tar file insted of uploading the image. We do this because some devices that will need the image will not always have an internet connection
